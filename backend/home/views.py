...
def login(request):
    ...
    try:
        user = Student.objects.get(email=data['email'])
        if user.password == data['password']:
            ...
            return JsonResponse({
                ...
                'specialite_bac': user.specialite_bac,
                'annee_obtention_bac': user.annee_obtention_bac,
                'mention_bac': user.mention_bac,
                'diplome_obtenu': user.diplome_obtenu,
                'specialite_diplome': user.specialite_diplome,
                'annee_obtention_diplome': user.annee_obtention_diplome,
                'mention_diplome': user.mention_diplome,
                'prof': user.prof,
                'authenticated': user.verfed,
                'modules': module_data
            })
        else:
...
def index(request):
    return render(request, 'index.html')  # Disable cross
